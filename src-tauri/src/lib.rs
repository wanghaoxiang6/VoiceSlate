...
async fn test_stt_connection(
    ...
    if api_key.is_empty() && provider != "local-whisper" && provider != "cloud-opus" {
        return Ok(false);
    }
    ...
    match provider.as_str() {
        "cloud-opus" => {
            let client = reqwest::Client::builder()
                ...
                .map_err(|e| e.to_string())?;
            let resp = client
                .get("http://127.0.0.1:8788/api/stt/providers?provider=cloud-opus")
...
#[tauri::command]
async fn get_stt_correction_suggestions(
    state: tauri::State<'_, storage::HistoryStore>,
    min_count: Option<i64>,
    limit: Option<u32>,
) -> Result<Vec<storage::SttCorrectionSuggestion>, String> {
    state
        .list_stt_correction_suggestions(min_count.unwrap_or(1), limit.unwrap_or(20))
        .await
        .map_err(|e| e.to_string())
}

#[tauri::command]
async fn mark_stt_correction_suggestion(
    state: tauri::State<'_, storage::HistoryStore>,
    id: i64,
    status: String,
) -> Result<(), String> {
    state
        .mark_stt_correction_suggestion(id, &status)
        .await
        .map_err(|e| e.to_string())
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_stt_correction_suggestions,
            mark_stt_correction_suggestion,
            get_history_stats,
            clear_history,
            get_dictionary,
