...
fn apply_stt_corrections(text: &str, config: &storage::AppConfig) -> String {
    if !config.stt_correction_enabled {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self
        ...
        if config_data.stt_api_key.is_empty()
            && !matches!(
                config_data.stt_provider.as_str(),
                "cloud" | "cloud-opus" | "local-whisper"
            )
        {
        ...
        *self
        ...
        if let Some((llm_config, provider)) = pre_llm {
            ...
            let req = PolishRequest {
                raw_text: corrected_stt_text.clone(),
...
mod tests {
    ...
    #[test]
    fn stt_corrections_are_explicit_and_ordered() {
        let config = AppConfig {
            stt_correction_enabled: true,
            stt_corrections: vec![SttCorrection {
                from: "字幕 by 索兰娅".to_string(),
                to: "非常多错误".to_string(),
                enabled: true,
            }],
            ..AppConfig::default()
        ...
        assert_eq!(
            apply_stt_corrections("这里有字幕 by 索兰娅", &config),
            "这里有非常多错误"
        );
    }

    #[test]
    fn stt_corrections_do_not_run_when_disabled() {
        let config = AppConfig {
