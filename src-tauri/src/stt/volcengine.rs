...
const MIN_DURATION_FOR_STANDARD_FALLBACK_SECS: f64 = 0.8;
...
async fn transcribe_with_standard_api(
    client: &reqwest::Client,
    config: &SttConfig,
    wav_data: &[u8],
    ...
    let submit_api_status = submit_resp
        .headers()
        .get("X-Api-Status-Code")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("")
        .to_string();
    let submit_api_message = submit_resp
        .headers()
        .get("X-Api-Message")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("")
...
impl SttProvider for VolcengineFlashProvider {
    ...
    async fn disconnect(&mut self) -> Result<Option<String>> {
        ...
        if !api_status.is_empty() && api_status != "20000000" {
            if api_status == "20000003" || api_status == "45000002" {
                tracing::warn!(
                    "Volcengine Flash ASR returned silence/empty audio (status={}, message={})",
                    api_status,
                    api_message
                );
                return Ok(None);
            ...
            anyhow::bail!(
                "Volcengine Flash ASR returned non-success status ({}): {}",
                api_status,
                api_message
            );
        }
        ...
        if audio_len_secs >= MIN_DURATION_FOR_STANDARD_FALLBACK_SECS {
            tracing::warn!(
                "Volcengine Flash ASR returned empty text for {:.1}s audio, retrying with standard mode",
                audio_len_secs
            );
            match transcribe_with_standard_api(&self.client, &config, &wav_data).await {
                Ok(Some(fallback_text)) if !fallback_text.trim().is_empty() => {
                    tracing::info!(
                        "Volcengine Standard fallback recovered {} chars",
                        fallback_text.len()
                    );
                    return Ok(Some(fallback_text));
                }
                Ok(_) => {
                    tracing::warn!("Volcengine Standard fallback also returned empty text");
                }
                Err(err) => {
                    tracing::warn!("Volcengine Standard fallback failed: {}", err);
                }
            }
        }
...
impl SttProvider for VolcengineStandardProvider {
    ...
    async fn disconnect(&mut self) -> Result<Option<String>> {
        ...
        let wav_data = WhisperCompatProvider::build_wav(&self.audio_buffer, config.sample_rate);
        self.audio_buffer.clear();
        transcribe_with_standard_api(&self.client, &config, &wav_data).await
    }
