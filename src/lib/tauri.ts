...
export interface SttCorrectionSuggestion {
  id: number
  wrong_text: string
  correct_text: string
  source_count: number
...
export async function getSttCorrectionSuggestions(
  minCount = 1,
  limit = 20,
): Promise<SttCorrectionSuggestion[]> {
  return invoke('get_stt_correction_suggestions', { minCount, limit })
}

export async function markSttCorrectionSuggestion(
  id: number,
  status: 'accepted' | 'ignored' | 'pending',
): Promise<void> {
  return invoke('mark_stt_correction_suggestion', { id, status })
}
